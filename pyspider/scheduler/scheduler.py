...
class Scheduler(object):
    ...
    def _check_task_done(self):
        ...
        try:
            while True:
                if (cnt + 1) % 100 == 0:
                    logger.debug("scheduler task done loop times: %s", cnt)
    ...
    def _load_put_task(self, project, taskid):
        try:
            ...
            if task is not None and task.get('page_num') == None:
                task['page_num'] = 0
    ...
    def run(self):
        ...
        while not self._quit:
            ...
            try:
                ...
                self.run_once()
                self._exceptions = 0
                logger.debug("scheduler end loop, current loop times: %s"%(loop_times))
...
class ThreadBaseScheduler(Scheduler):
    def __init__(self, threads=4, *args, **kwargs):
        ...
        self._projectcache = self.projectcache
