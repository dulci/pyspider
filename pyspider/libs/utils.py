...
def is_all_url_exists(response, url_list):
    ...
    for index in range(len(url_list)-1, -1, -1):
        ...
        if not task_db.get_task(response.project, md5string(url_list[index])) and not [result for result in result_db.select(response.project, group, taskid=md5string(url_list[index]))]:
            all_exists = False
            continue
        url_list.pop(index)
    return all_exists
