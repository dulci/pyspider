...
def is_all_url_exists(response, url_list):
    ...
    for index in range(len(url_list)-1, -1, -1):
        ...
        logging.info('will be fetch task %s:%s:%s:%s'%(response.project,url_list[index],group,md5string(url_list[index])))
        logging.info('%s:%s'%(task_db.get_task(response.project, md5string(url_list[index])), [result for result in result_db.select(response.project, group, taskid=md5string(url_list[index]), fields=['taskid'])]))
