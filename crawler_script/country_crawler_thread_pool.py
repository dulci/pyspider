...
#resultdb = ResultDB('mysql+mysqlconnector://crawlergov:X2xMGSJjLKC490%@10.125.4.148:8635/crawler_gov')
#proxypooldb = Proxypooldb('10.125.4.64',6379,'www.zx.c0m',7)
resultdb = ResultDB('mysql+mysqlconnector://gcj_admin:test@192.168.133.176:3306/caijia_zbxxcl')
proxypooldb = Proxypooldb('192.168.133.176', 6379, None, 14)
...
def on_start(url, method=None, data=None, cookies=None, max_page=None, retry_num=0):
    company_list_pyquery, cookies = get('http://jzsc.mohurd.gov.cn/dataservice/query/comp/list', method=method, data=data, cookies=cookies)
    ...
    if company_list_pyquery is None:
        if retry_num < 2:
            ...
            time.sleep(240)
            on_start(url,method,data,cookies,max_page,retry_num+1)
            return
        else:
    ...
    for each in company_list_pyquery('.cursorDefault tr').items():
        # start = time.time()
        company_page('http://jzsc.mohurd.gov.cn%s'%(each.find('a').attr.href),{'company_code': each.find('td:nth-child(2)').text(), 'company_name': each.find('td:nth-child(3)').text(), 'company_id':md5string(each.find('td:nth-child(3)').text()), 'org_leader':each.find('td:nth-child(4)').text(), 'region':each.find('td:nth-child(5)').text()})
        # end = time.time()
    ...
    if int(page_count) > int(page):
        on_start('http://jzsc.mohurd.gov.cn/dataservice/query/comp/list',method='post',data={'$total':total,'$reload':reload,'$pg':int(page)+1,'$pgsz':page_size},cookies=cookies,max_page=max_page,retry_num=0)
...
def query_company():
    ...
    for page in range(page_count):
        ...
        for one in session.execute('select ent_name from enterprise_base limit %s offset %s'%(page_size, page*page_size)):
            company_pyquery, cookies= get('http://jzsc.mohurd.gov.cn/dataservice/query/comp/list', method='post', data={'complexname': one[0]})
            for each in company_pyquery('.cursorDefault tr').items():
                if each.find('a'):
                    company_page('http://jzsc.mohurd.gov.cn%s' % (each.find('a').attr.href),
...
def query_company_by_file():
    ...
    with open("D:\company.json", encoding="UTF-8") as f:
        ...
        for line in f.readlines():
            ...
            company_pyquery, cookies = get('http://jzsc.mohurd.gov.cn/dataservice/query/comp/list',method='post', data={'complexname': line_json['企业信息']['企业名称']})
            if company_pyquery is None:
                continue
            for each in company_pyquery('.cursorDefault tr').items():
                if each.find('a'):
                    pool.submit(company_page, 'http://jzsc.mohurd.gov.cn%s' % (each.find('a').attr.href), save={'company_code': each.find('td:nth-child(2)').text(),
                                       'company_name': each.find('td:nth-child(3)').text(),
                                       'company_id': md5string(each.find('td:nth-child(3)').text()),
                                       'org_leader': each.find('td:nth-child(4)').text(),
                                       'region': each.find('td:nth-child(5)').text()})
...
def company_page(url, save={}, retry_num=0):
    ...
    qualifications = list()
    qualification_page('http://jzsc.mohurd.gov.cn/dataservice/query/comp/caDetailList/%s' % (url.split('/')[-1]), qualifications=qualifications)
    company_info['qualifications'] = qualifications
    projects = list()
    project_list('http://jzsc.mohurd.gov.cn/dataservice/query/comp/compPerformanceListSys/%s'%(url.split('/')[-1]), projects=projects)
    company_info['projects'] = projects
    persons = list()
    person_list('http://jzsc.mohurd.gov.cn/dataservice/query/comp/regStaffList/%s'%(url.split('/')[-1]), persons=persons)
    company_info['persons'] = persons
    company_info['company_break_promises'] = company_break_promises('http://jzsc.mohurd.gov.cn/dataservice/query/comp/compCreditRecordList/%s/0' % (url.split('/')[-1]))
    company_info['black_list'] = black_list('http://jzsc.mohurd.gov.cn/dataservice/query/comp/compCreditBlackList/%s'%(url.split('/')[-1]))
    company_info['lose_credits'] = lose_credits('http://jzsc.mohurd.gov.cn/dataservice/query/comp/compPunishList/%s'%(url.split('/')[-1]))
    resultdb.save('country',company_info['company_id'],url,company_info,'self_crawler')
    return company_info

def qualification_page(url, method=None, data=None, cookies=None, qualifications=[]):
    qualification_pyquery, cookies = get(url, method, data, cookies)
    ...
    if qualification_pyquery('.clearfix'):
        ...
        if int(page_count) > int(page):
            qualification_page(url, method='post', data={'$total':total,'$reload':reload,'$pg':int(page)+1,'$pgsz':page_size},cookies=cookies, qualifications=qualifications)

def project_list(url, method=None, data=None, cookies=None, projects=[]):
    project_list_pyquery, cookies = get(url,method=method,data=data,cookies=cookies)
    ...
    for each in project_list_pyquery('.pro_table_box tbody tr').items():
        if not each.find('td[data-header]'):
            continue
        #project_info = project_page('http://jzsc.mohurd.gov.cn/dataservice/query/project/projectDetail/%s' % (each.find('td:nth-child(2)').text()))
    ...
    if project_list_pyquery('.clearfix'):
        ...
        if int(page_count) > int(page):
            project_list(url, method='post', data={'$total':total,'$reload':reload,'$pg':int(page)+1,'$pgsz':page_size},cookies=cookies, projects=projects)

def project_page(url):
    project_pyquery, cookies = get(url)
    ...
    for one in project_pyquery('#tab_ztb tr').items():
        if one.find('td:nth-last-child(1)').find('a'):
            ...
            bids.append(bid)
            # bid_pyquery, cookies = get('http://jzsc.mohurd.gov.cn%s' % (one.find('td:nth-last-child(1)').find('a').attr('data-url')), cookies=cookies)
    ...
    for one in project_pyquery('#tab_sgtsc tr').items():
        if one.find('td:nth-last-child(1)').find('a'):
            ...
            # drawing_review_pyquery, cookies = get('http://jzsc.mohurd.gov.cn%s' % (one.find('td:nth-last-child(1)').find('a').attr('data-url')), cookies=cookies)
    ...
    for one in project_pyquery('#tab_htba tr').items():
        if one.find('td:nth-last-child(1)').find('a'):
            contract = {'contract_type': one.find('td[data-header=合同类别]').text(),
                        ...
                        'detail_url': 'http://jzsc.mohurd.gov.cn%s' % (one.find('td:nth-last-child(1)').find('a').attr('data-url'))}
            # contract_pyquery, cookies = get('http://jzsc.mohurd.gov.cn%s' % (one.find('td:nth-last-child(1)').find('a').attr('data-url')), cookies=cookies)
    ...
    for one in project_pyquery('#tab_sgxk tr').items():
        if one.find('td:nth-last-child(1)').find('a'):
            construction_permit = {'construction_permit_code': one.find('td[data-header=施工许可证编号]').text(),
                                   ...
                                   'detail_url': 'http://jzsc.mohurd.gov.cn%s' % (one.find('td:nth-last-child(1)').find('a').attr('data-url'))}
            # construction_permit_pyquery, cookies = get('http://jzsc.mohurd.gov.cn%s' % (one.find('td:nth-last-child(1)').find('a').attr('data-url')), cookies=cookies)
    ...
    for one in project_pyquery('#tab_jgysba tr').items():
        if one.find('td:nth-last-child(1)').find('a'):
            completed_review = {'completed_review_code': one.find('td[data-header=竣工备案编号]').text(),
                                ...
                                'detail_url': 'http://jzsc.mohurd.gov.cn%s' % (one.find('td:nth-last-child(1)').find('a').attr('data-url'))}
            # completed_review_pyquery, cookies = get('http://jzsc.mohurd.gov.cn%s' % (one.find('td:nth-last-child(1)').find('a').attr('data-url')), cookies=cookies)
...
def person_list(url, method=None, data=None, cookies=None, persons=[]):
    person_list_pyquery, cookies = get(url,method=method,data=data,cookies=cookies)
    ...
    for each in person_list_pyquery('.pro_table_box tbody tr').items():
        if not each.find('td[data-header]'):
            continue
        #person_info = person_page('http://jzsc.mohurd.gov.cn%s' % (re.search('href=\'([\s\S]+?)\'', each.find('td:nth-child(2) a').attr.onclick).group(1)))
    ...
    if person_list_pyquery('.clearfix'):
        ...
        if int(page_count) > int(page):
            person_list(url, method='post', data={'$total':total,'$reload':reload,'$pg':int(page)+1,'$pgsz':page_size},cookies=cookies, persons=persons)

def person_page(url):
    person_pyquery, cookies = get(url)
    ...
    #     change_info_pyquery, cookies = get('http://jzsc.mohurd.gov.cn/dataservice/query/staff/staffWorkRecordList/%s' % (url.split('/')[-1]), cookies=cookies)
    ...
    # # 不良行为
    # break_promises = list()
    # break_promise_pyquery, cookies = get('http://jzsc.mohurd.gov.cn/dataservice/query/staff/staffCreditRecordList/%s/0' % (url.split('/')[-1]), cookies=cookies)
...
def company_break_promises(url):
    # 企业不良行为
    company_break_promises = list()
    company_break_promise_pyquery, cookies = get(url)
...
def black_list(url):
    # 黑名单
    black_list = list()
    black_list_pyquery, cookies = get(url)
...
def lose_credits(url):
    lose_credits = list()
    lose_credits_pyquery, cookies = get(url)
