...
def get(url,method=None,data=None,cookies=None,timeout=60,retry_num=0, proxy=None):
    try:
        ...
        if method and method.lower() == 'post':
            response = requests.post(url,data=data,cookies=cookies,headers=headers,timeout=timeout, verify=False)
        else:
            response = requests.get(url,params=data,cookies=cookies,headers=headers,timeout=timeout, verify=False)
...
#company_types = ['02','03','04','05','06','07','09','11','13','16','cgr','zfcgzrr']
...
def company_list(url, method=None, data=None, cookies=None, count=0):
    companies_pyquery, cookies = get(url,method,data,cookies)
    logger.error('company_type is %s, page is %s'%(data['qyyj_qylx'], data['page'])) 
    for each in companies_pyquery('.wsbs-table tr').items():
        if each.find('td'):
            logger.error(each.find('a').parent().prev().text())
            if session.execute('select count(1) from crawler_content_result_record where taskid = \'%s\''%(each.find('a').parent().prev().text())).scalar() > 0:
                continue
            count += 1
            logger.error(count)
    ...
    pages = int(re.search('共(\d+)页', companies_pyquery('.pagination ul li:nth-last-child(1)').text()).group(1))
    page = data['page']
    if (pages-page) > 0:
        data['page'] = data['page'] + 1
        company_list(url,method=method,data=data,cookies=cookies,count=count)

def company_crawler(url, company_info):
    company_pyquery, cookies = get(url)
    if company_pyquery is None:
        logger.error('company request error')
        return False
    # 企业类型
    company_types = list()
        ...
        for each in artisans['data']:
            ...
            try:
                ...
                for q in json.loads(requests.post('http://qyk.gzcc.gov.cn/qyww/json/',data={'arguments': json.dumps([0, 1000, {'rybh': each['rybh']}]),'method': 'findTQyZyjsryzcxxInfo','service': 'TQyZyjsryzcxxBS'}).content)['data']:
                    artisan['qualifications'].append({'type': q['zclx'], 'level': q['zcdj'], 'code': q['zgzsh'], 'failure_time': q['zcyxq']})
                # 安全考核证书
